...
var onTagClick = function () {
    ...
    //     });
    // }
    var tag_info = $(this).next().text();

    $('.result').removeClass('custom-hidden');

    if (tag_info != 'All') {
        $('.result').each(function () {
            let defect = $(this).children('.date').text();
            // console.log(defect);
            if (defect != tag_info) {
...
function handleTagTrigger() {
    ...
    var defectTags = $('#object-value-holder input[type="checkbox"]');
    defectTags.click(onTagClick);
